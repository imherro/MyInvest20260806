...
function Market() {
  ...
  return <>
    {header}
    <div className="prototype-banner"><span>VERIFIED DATA SNAPSHOT</span><b>当前为双源校验真实快照</b><em>{data.source.providers.join(" + ")} · {data.source.apis.join(" / ")}</em><small>信息截止 {display(data.asOf)} · 质量 {display(data.dataQuality.grade)} · 覆盖率 {display(data.dataQuality.coverage)}</small></div>
