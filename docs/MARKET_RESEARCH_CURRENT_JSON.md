...
当前11/14项为真实数据：F1、L1、L2、L3、L4、L5、B1、B2、B3、B4、B5；其余3项为 `pending`。F1只使用最近已结束季度中截至 `asOf` 已有合法公告日的公司最新记录，计算有值 `netprofit_yoy` 的中位数；公告日期缺失记录排除，当前不是全A总利润增长率。B1至B5共享同一 `trade_date`；B2复用B4的同一次 `daily_basic` 请求，只对非空、有限且非负的 `dv_ttm` 有值样本按 `total_mv` 加权，空值不视为0，当前不是“股息率－无风险利率”。B4仅汇总该日实际返回记录的总市值，未接GDP，不是总市值/GDP或巴菲特指标。F/L/B一级评分、联合市场判断、投资含义和仓位倾向仍为 `null`，PIT状态仍为“待接入”。
