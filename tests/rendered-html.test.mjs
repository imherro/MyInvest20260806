...
test("normalizes only complete cn_m M1/M2 rows for L2", async () => {
  const { normalizeCnMRows } = await l2HistoryGenerator(); const rows = normalizeCnMRows([{ month: "201502", m1_yoy: "5.6", m2_yoy: "8.8", m1: 99, m1_mom: 88 }, { month: "201501", m1_yoy: 4.7, m2_yoy: 8.1 }], "201501", "201502");
  assert.deepEqual(rows, [{ month: "201501", m1_yoy: 4.7, m2_yoy: 8.1 }, { month: "201502", m1_yoy: 5.6, m2_yoy: 8.8 }]); assert.doesNotMatch(JSON.stringify(rows), /"m1"|m1_mom/); assert.throws(() => normalizeCnMRows([], "201501", "201502"), /no rows/); assert.throws(() => normalizeCnMRows([{ month: "201501", m1_yoy: 1, m2_yoy: 2 }], "201501", "201502"), /missing or non-contiguous/); assert.throws(() => normalizeCnMRows([{ month: "201501", m1_yoy: 1, m2_yoy: 2 }, { month: "201501", m1_yoy: 1, m2_yoy: 2 }], "201501", "201501"), /duplicate month/); assert.throws(() => normalizeCnMRows([{ month: "201412", m1_yoy: 1, m2_yoy: 2 }], "201501", "201501"), /outside requested range/);
  for (const month of ["201500", "201513", "2015-01", "x"]) assert.throws(() => normalizeCnMRows([{ month, m1_yoy: 1, m2_yoy: 2 }], "201501", "201501"), /invalid month/); for (const field of ["m1_yoy", "m2_yoy"]) for (const value of [null, "", Number.NaN, Infinity, "bad"]) assert.throws(() => normalizeCnMRows([{ month: "201501", m1_yoy: 1, m2_yoy: 2, [field]: value }], "201501", "201501"), new RegExp(`invalid ${field}`));
});

test("builds deterministic 139-point L2 history from the B3 timeline", async () => {
  const { buildL2MonthlyHistory, shiftMonth } = await l2HistoryGenerator(); const b3 = await b3HistoryData(); const rows = Array.from({ length: 139 }, (_, index) => ({ month: shiftMonth("201412", index), m1_yoy: index / 10, m2_yoy: index / 20 }));
  const first = buildL2MonthlyHistory(b3, [...rows].reverse(), "2026-08-17"); const second = buildL2MonthlyHistory(b3, rows, "2026-08-17"); assert.equal(first.points.length, 139); assert.equal(JSON.stringify(first), JSON.stringify(second)); assert.deepEqual(first.sourceQuery, { api: "cn_m", startM: "201412", endM: "202606", fields: ["month", "m1_yoy", "m2_yoy"] }); assert.match(first.sourceSnapshotSha256, /^[a-f0-9]{64}$/); assert.equal(first.points[0].period, "201412"); assert.equal(first.points.at(-1).period, "202606");
  for (let index = 0; index < first.points.length; index += 1) { const point = first.points[index]; assert.equal(point.asOf, b3.points[index].asOf); assert.ok(point.releaseDate <= point.asOf); assert.equal(point.releaseDateQuality, "conservative_proxy"); assert.equal(point.pitScope, "release_lag_only"); assert.equal(point.valueVintage, "latest_available_snapshot"); assert.deepEqual(point.sourceFields, ["m1_yoy", "m2_yoy"]); assert.deepEqual(point.units, { m1Yoy: "pct", m2Yoy: "pct", gap: "pct_point" }); assert.equal(point.m1Yoy, index / 10); assert.equal(point.m2Yoy, index / 20); assert.equal(point.gap, index / 20); assert.equal("value" in point, false); } assert.doesNotMatch(JSON.stringify(first), /m1_mom|"score"|"trend"|"normalized"/);
});
...
test("rejects malformed B3 schedules and silent cn_m revisions", async () => {
  ...
  const legacy = { points: [{ period: "201501", value: 4, source: "tushare", dataset: "cn_m", sourceField: "m1_yoy" }, { period: "201502", value: null, source: "placeholder" }] }; assert.doesNotThrow(() => assertNoSilentRevision(legacy, [{ month: "201501", m1_yoy: 4, m2_yoy: 8 }])); assert.throws(() => assertNoSilentRevision(legacy, [{ month: "201501", m1_yoy: 4.1, m2_yoy: 8 }]), /historical revision.*201501/);
  const current = { points: [{ period: "201501", m1Yoy: 4, m2Yoy: 8, gap: -4, source: "tushare", dataset: "cn_m", sourceFields: ["m1_yoy", "m2_yoy"] }] }; assert.doesNotThrow(() => assertNoSilentRevision(current, [{ month: "201501", m1_yoy: 4, m2_yoy: 8 }])); assert.throws(() => assertNoSilentRevision(current, [{ month: "201501", m1_yoy: 4, m2_yoy: 8.1 }]), /historical revision.*201501/);
});

test("checked-in L2 history is complete and explicitly conservative PIT", async () => {
  const [l2, b3] = await Promise.all([l2HistoryData(), b3HistoryData()]); assert.equal(l2.points.length, 139); assert.equal(l2.requestedAsOf, b3.requestedAsOf); assert.equal(l2.range.startPeriod, "201412"); assert.equal(l2.range.endPeriod, "202606"); assert.match(l2.sourceSnapshotSha256, /^[a-f0-9]{64}$/); assert.equal(l2.points[0].gap, -9); assert.equal(l2.points.at(-1).gap, -4); for (let index = 0; index < l2.points.length; index += 1) { const point = l2.points[index]; assert.equal(point.asOf, b3.points[index].asOf); assert.ok(Number.isFinite(point.m1Yoy)); assert.ok(Number.isFinite(point.m2Yoy)); assert.ok(Number.isFinite(point.gap)); assert.equal(Number((point.m1Yoy - point.m2Yoy).toFixed(12)), point.gap); assert.equal("value" in point, false); assert.ok(point.releaseDate <= point.asOf); assert.equal(point.releaseDateQuality, "conservative_proxy"); assert.equal(point.pitScope, "release_lag_only"); assert.equal(point.valueVintage, "latest_available_snapshot"); }
});

test("repaired L2 exactly preserves the three legacy cn_m series and PIT metadata", async () => {
  const baseline = "c8197eab8683db4144c146ac506ffc0c25db4741"; const show = relative => { const result = spawnSync("git", ["show", `${baseline}:${relative}`], { cwd: fileURLToPath(new URL("..", import.meta.url)), encoding: "utf8" }); assert.equal(result.status, 0, result.stderr); return JSON.parse(result.stdout); };
  const [current, oldL2, oldL3, oldL4] = await Promise.all([l2HistoryData(), Promise.resolve(show("public/data/market-research/history/l2.json")), Promise.resolve(show("public/data/market-research/history/l3.json")), Promise.resolve(show("public/data/market-research/history/l4.json"))]); assert.equal(current.points.length, 139);
  for (let index = 0; index < current.points.length; index += 1) { const point = current.points[index]; assert.equal(point.m1Yoy, oldL2.points[index].value); assert.equal(point.m2Yoy, oldL3.points[index].value); assert.equal(point.gap, oldL4.points[index].value); for (const field of ["asOf", "period", "releaseDate", "releaseDateQuality", "pitScope", "valueVintage"]) assert.equal(point[field], oldL2.points[index][field]); }
});
...
test("L4 field audit validates the F0 artifact with a cross-platform canonical hash before network", async () => {
  const { EXPECTED_F0_SHA256, buildFieldAudit, main, validateF0Input } = await l4FieldAuditGenerator();
  ...
  const variants = [lf, lf.replace(/\n/g, "\r\n"), lf.replace(/\n/g, "\r")]; const parsed = variants.map(text => validateF0Input(Buffer.from(text))); assert.equal(parsed.every(value => value.reports.length === 119), true); assert.equal(parsed.every(value => JSON.stringify(value) === JSON.stringify(parsed[0])), true); assert.equal(parsed.every(value => JSON.stringify(buildFieldAudit(value, [])) === JSON.stringify(buildFieldAudit(parsed[0], []))), true);
  const outputs = []; for (const text of variants) { const requested = []; let output; await main({ readInput: async () => Buffer.from(text), fetchHtml: async report => { requested.push(report.normalizedUrl); const month = Number(report.period.slice(4, 6)); const marker = month === 3 ? "一季度" : month === 6 ? "上半年" : month === 9 ? "前三季度" : month === 12 ? "全年" : `1-${month}月累计`; const [year, dateMonth, day] = report.publicationDate.split("-").map(Number); return `<html><head><meta name="ArticleTitle" content="${report.title}"></head><body><div class="laiyuan">发布日期：${year}年${dateMonth}月${day}日</div><p>${marker}，全国一般公共预算收入1亿元，同比增长1%。${marker}，全国一般公共预算支出1亿元，同比增长1%。</p></body></html>`; }, writeOutput: async (_target, value) => { output = JSON.stringify(value); } }); assert.equal(requested.length, 119); assert.deepEqual([...new Set(requested)].sort(), parsed[0].reports.map(report => report.normalizedUrl).sort()); outputs.push(output); } assert.equal(outputs.every(output => output === outputs[0]), true);
  assert.throws(() => validateF0Input(Buffer.from(lf.slice(0, -1))), /SOURCE_AUDIT_TERMINAL_NEWLINE_INVALID/); assert.throws(() => validateF0Input(Buffer.from(`${lf}\n`)), /SOURCE_AUDIT_TERMINAL_NEWLINE_INVALID/); assert.throws(() => validateF0Input(Buffer.concat([Buffer.from([0xef, 0xbb, 0xbf]), Buffer.from(lf)])), /SOURCE_AUDIT_BOM_FORBIDDEN/); assert.throws(() => validateF0Input(Buffer.from([0xff])), /SOURCE_AUDIT_ENCODING_INVALID/);
  for (const changed of [lf.replace('"schemaVersion": 1', '"schemaVersion": 2'), lf.replace('  "schemaVersion": 1,\n  "indicatorId": "L4",', '  "indicatorId": "L4",\n  "schemaVersion": 1,'), lf.replace('"schemaVersion": 1', '"schemaVersion" : 1'), lf.replace('"title": "', '"title": "changed '), lf.replace(/\n}\n$/, ',\n  "extra": true\n}\n')]) assert.throws(() => validateF0Input(Buffer.from(changed)), /SOURCE_AUDIT_CANONICAL_HASH_MISMATCH/);
  let networkCount = 0; let writeCount = 0; await assert.rejects(main({ readInput: async () => Buffer.from(lf.replace('"schemaVersion": 1', '"schemaVersion": 2')), fetchHtml: async () => { networkCount += 1; }, writeOutput: async () => { writeCount += 1; } }), /SOURCE_AUDIT_CANONICAL_HASH_MISMATCH/); assert.equal(networkCount, 0); assert.equal(writeCount, 0);
  await assert.rejects(main({ readInput: async () => Buffer.from(lf), fetchHtml: async () => { throw new Error("network failure"); }, writeOutput: async () => { writeCount += 1; } }), /network failure/); assert.equal(writeCount, 0);
});
...
test("L4 field extraction binds explicit cumulative scopes and excludes every single-month match", async () => {
  const { amountToYi, auditReportFields, extractVisibleBlocks } = await l4FieldAuditGenerator(); assert.equal(amountToYi("1.25", "万亿元"), "12500"); assert.equal(amountToYi("140350", "亿元"), "140350");
  const report = { period: "202604", title: "2026年4月财政收支情况", normalizedUrl: "https://gks.mof.gov.cn/tongjishuju/202605/t.htm", publicationDate: "2026-05-20" }; const wrap = body => `<html><head><meta name="ArticleTitle" content="${report.title}"></head><body><div class="laiyuan">发布日期：2026年5月20日</div>${body}</body></html>`; const audit = body => auditReportFields(wrap(body), report);
  const mixed = audit(`<p>4月份，全国一般公共预算收入100亿元，同比增长99%；1—4月累计，全国一般公共预算收入600亿元，同比增长6%，同口径增长5%。4月当月，全国一般公共预算支出200亿元，同比下降88%；1至4月累计，全国一般公共预算支出1.25万亿元，同比增长4%。</p>`); assert.deepEqual(mixed.revenueCumYi.candidates.map(candidate => candidate.value), ["600"]); assert.deepEqual(mixed.revenueYoyPct.candidates.map(candidate => candidate.value), ["6", "5"]); assert.deepEqual(mixed.expenditureCumYi.candidates.map(candidate => candidate.value), ["12500"]); assert.deepEqual(mixed.expenditureYoyPct.candidates.map(candidate => candidate.value), ["4"]); assert.equal(mixed.revenueCumYi.status, "unique"); assert.equal(mixed.expenditureCumYi.status, "unique"); assert.equal(mixed.revenueYoyPct.status, "ambiguous_multiple_candidates");
  const reversed = audit(`<p>1-4月累计，全国一般公共预算收入600亿元，同比增长6%；4月份，全国一般公共预算收入100亿元，同比增长99%。1-4月累计，全国一般公共预算支出700亿元，同比增长7%；本月，全国一般公共预算支出200亿元，同比增长88%。</p>`); assert.deepEqual(reversed.revenueCumYi.candidates.map(candidate => candidate.value), ["600"]); assert.deepEqual(reversed.expenditureCumYi.candidates.map(candidate => candidate.value), ["700"]); assert.deepEqual(reversed.expenditureYoyPct.candidates.map(candidate => candidate.value), ["7"]); assert.equal(Object.values(reversed).every(field => field.status === "unique"), true);
  const inherited = audit(`<p>1-4月累计</p><p>全国一般公共预算收入600亿元，同比增长6%。</p><p>1-4月累计</p><p>全国一般公共预算支出700亿元，同比下降2%。</p>`); assert.deepEqual(inherited.revenueCumYi.candidates.map(candidate => candidate.value), ["600"]); assert.deepEqual(inherited.expenditureYoyPct.candidates.map(candidate => candidate.value), ["-2"]);
  const unresolved = audit(`<p>全国一般公共预算收入600亿元，同比增长6%。</p><p>1—3月累计，全国一般公共预算支出700亿元，同比增长7%。</p>`); assert.equal(unresolved.revenueCumYi.status, "missing"); assert.equal(unresolved.expenditureCumYi.status, "missing"); const crossed = audit(`<p>1-4月累计</p><h2>二、其他章节</h2><p>全国一般公共预算收入600亿元，同比增长6%。</p>`); assert.equal(crossed.revenueCumYi.status, "missing");
  const specialCases = [["202603", "一季度"], ["202606", "上半年"], ["202609", "前三季度"], ["202612", "全年"]]; for (const [period, marker] of specialCases) { const specialReport = { ...report, period }; const specialHtml = wrap(`<p>${marker}，全国一般公共预算收入600亿元，同比增长6%。</p>`).replace(report.title, specialReport.title); assert.equal(auditReportFields(specialHtml, specialReport).revenueCumYi.status, "unique"); }
  const brHtml = `<p>1-4月累计，<br>全国一般公共预算收入600亿元，同比增长6%。</p>`; assert.equal(extractVisibleBlocks(wrap(brHtml)).some(block => block === "全国一般公共预算收入600亿元，同比增长6%。"), true); assert.equal(audit(brHtml).revenueCumYi.status, "unique");
  for (const field of Object.values(mixed)) for (const candidate of field.candidates) { assert.equal(candidate.scopeType, "cumulative"); assert.equal(candidate.scopeEndPeriod, report.period); assert.equal(candidate.evidenceSha256, createHash("sha256").update(candidate.evidenceSentence).digest("hex")); assert.deepEqual(Object.keys(candidate), ["value", "unit", "basis", "scopeType", "scopeGranularity", "scopeSource", "scopeEndPeriod", "evidenceSentence", "evidenceSha256", "url", "publicationDate"]); }
});

test("L4 field extraction accepts explicit body years only as matching full-year scopes", async () => {
  const { auditReportFields } = await l4FieldAuditGenerator(); const makeReport = period => ({ period, title: `${period.slice(0, 4)}年财政收支情况`, normalizedUrl: `https://gks.mof.gov.cn/tongjishuju/${period}/t.htm`, publicationDate: "2025-01-30" }); const audit = (period, body) => { const report = makeReport(period); const html = `<html><head><meta name="ArticleTitle" content="${report.title}"></head><body><div class="laiyuan">发布日期：2025年1月30日</div><p>${body}</p></body></html>`; return auditReportFields(html, report); };
  const annual = audit("202412", "2024年，全国一般公共预算收入219702亿元，同比增长1.3%，同口径增长1.1%；2024年，全国一般公共预算支出284612亿元，同比增长3.6%。"); assert.equal(annual.revenueCumYi.status, "unique"); assert.equal(annual.expenditureCumYi.status, "unique"); assert.equal(annual.revenueYoyPct.status, "ambiguous_multiple_candidates"); for (const field of Object.values(annual)) for (const candidate of field.candidates) { assert.equal(candidate.scopeType, "cumulative"); assert.equal(candidate.scopeGranularity, "full_year"); assert.equal(candidate.scopeSource, "explicit_body_year"); assert.equal(candidate.scopeEndPeriod, "202412"); }
  assert.equal(audit("202412", "2023年，全国一般公共预算收入1亿元，同比增长1%。").revenueCumYi.status, "missing"); assert.equal(audit("202411", "2024年，全国一般公共预算收入1亿元，同比增长1%。").revenueCumYi.status, "missing");
  for (const text of ["比2024年增长1%，全国一般公共预算收入1亿元。", "与2024年相比，全国一般公共预算收入1亿元。", "2024年同期，全国一般公共预算收入1亿元。", "全国一般公共预算收入1亿元，2024年同比增长1%。"]) assert.equal(audit("202412", text).revenueCumYi.status, "missing");
  assert.equal(audit("202412", "2024年，12月份，全国一般公共预算收入1亿元，同比增长1%。").revenueCumYi.status, "missing"); assert.equal(audit("202412", "全国一般公共预算收入1亿元，同比增长1%。").revenueCumYi.status, "missing");
});
...
test("checked-in L4 field audit is deterministic coverage-only evidence and remains outside joint", async () => {
  ...
  assert.deepEqual(audit.coverage, { revenueCumYi: { unique: 119, missing: 20, ambiguous_multiple_candidates: 0, invalid: 0 }, revenueYoyPct: { unique: 106, missing: 20, ambiguous_multiple_candidates: 13, invalid: 0 }, expenditureCumYi: { unique: 119, missing: 20, ambiguous_multiple_candidates: 0, invalid: 0 }, expenditureYoyPct: { unique: 116, missing: 20, ambiguous_multiple_candidates: 3, invalid: 0 } }); assert.equal(audit.months.filter(month => month.monthStatus === "complete_unique").length, 105); assert.equal(audit.months.filter(month => month.monthStatus === "ambiguous_fields").length, 14);
  for (const month of audit.months) { if (month.sourceStatus === "missing_official_report") { assert.equal(month.monthStatus, "missing_official_report"); for (const field of Object.values(month.fields)) assert.deepEqual(field, { status: "missing", candidates: [] }); } else { assert.equal(month.releaseDateQuality, "official_report_date"); assert.equal(month.pitScope, "release_lag_only"); assert.equal(month.valueVintage, "latest_available_snapshot"); for (const field of Object.values(month.fields)) for (const candidate of field.candidates) { assert.equal(candidate.scopeType, "cumulative"); assert.equal(typeof candidate.scopeGranularity, "string"); assert.equal(typeof candidate.scopeSource, "string"); assert.equal(candidate.scopeEndPeriod, month.period); assert.doesNotMatch(candidate.evidenceSentence, /(?:\d{1,2}\s*月份|\d{1,2}\s*月当月|当月|本月|单月)/); if (candidate.scopeSource === "explicit_body_year") { assert.equal(candidate.scopeGranularity, "full_year"); assert.equal(month.period.endsWith("12"), true); } } } for (const forbidden of ["value", "pulse", "direction", "threshold", "score"]) assert.equal(Object.hasOwn(month, forbidden), false); }
...
test("source hash policy makes LF CRLF and CR joint inputs byte-stable", async () => {
  ...
  const lfJoint = JSON.stringify(buildJointHistory(inputs, variants.lf)); assert.equal(JSON.stringify(buildJointHistory(inputs, variants.crlf)), lfJoint); assert.equal(JSON.stringify(buildJointHistory(inputs, variants.cr)), lfJoint); assert.equal(variants.lf.L2.sha256, sourceFileSha256(await readFile(fileURLToPath(new URL("../public/data/market-research/history/l2.json", import.meta.url)))));
});

test("offline joint history builds deterministic 12-of-14 snapshots without scores", async () => {
  ...
  assert.equal(first.snapshots.length, 139); assert.equal(JSON.stringify(first), JSON.stringify(second)); assert.deepEqual(first.intentionallyMissing, ["L3", "L4"]); assert.deepEqual(INTENTIONALLY_MISSING, ["L3", "L4"]); assert.equal(first.scorePolicy, "require_14_of_14");
  for (let index = 0; index < first.snapshots.length; index += 1) { const snapshot = first.snapshots[index]; assert.equal(Object.keys(snapshot.indicators).length, 12); assert.deepEqual(Object.keys(snapshot.indicators), [...INCLUDED_INDICATORS].sort()); assert.equal(snapshot.indicators.B3.asOf, snapshot.asOf); assert.equal(snapshot.indicators.F1.medianNetProfitYoy, inputs.F1.points[index].medianNetProfitYoy); assert.deepEqual(snapshot.indicators.L2, Object.fromEntries(Object.entries(inputs.L2.points[index]).sort(([a], [b]) => a.localeCompare(b)))); assert.equal(snapshot.indicators.L3, undefined); assert.equal(snapshot.indicators.L4, undefined); assert.deepEqual(snapshot.coverage, { B: { missing: [], present: 5, required: 5, status: "complete" }, F: { missing: [], present: 4, required: 4, status: "complete" }, L: { missing: ["L3", "L4"], present: 3, required: 5, status: "incomplete" }, overall: { present: 12, required: 14, status: "incomplete" } }); assert.equal(snapshot.scoreStatus, "not_computed_incomplete_inputs"); assert.equal(snapshot.aggregateScore, null); assert.equal(snapshot.jointState, null); }
...
test("offline joint history generator is network-free and reads only frozen inputs", async () => {
  ...
  assert.doesNotMatch(source, /TUSHARE_TOKEN|callTushare|pbc|https?:|\bfetch\b/i); assert.match(source, /INCLUDED_INDICATORS\.map/); assert.match(source, /"L2"/); assert.match(source, /INTENTIONALLY_MISSING = \["L3", "L4"\]/); assert.match(source, /readFile\(path\.resolve\(relative\)\)/); assert.doesNotMatch(source, /history\/l3\.json|history\/l4\.json|readdir|glob|forward.?fill|backward.?fill|child_process|execFile|core\.autocrlf|git show/i);
});

test("checked-in offline joint history preserves 139 exact snapshots without scores", async () => {
  const [joint, l2] = await Promise.all([jointHistoryData(), l2HistoryData()]); assert.equal(joint.snapshots.length, 139); assert.equal(joint.asOfCutoff, "2026-08-17"); assert.equal(joint.masterTimeline, "B3"); assert.equal(joint.sourceHashPolicy, "utf8_lf_normalized"); assert.deepEqual(joint.intentionallyMissing, ["L3", "L4"]);
  for (let index = 0; index < joint.snapshots.length; index += 1) { const snapshot = joint.snapshots[index]; assert.equal(Object.keys(snapshot.indicators).length, 12); assert.deepEqual(snapshot.indicators.L2, Object.fromEntries(Object.entries(l2.points[index]).sort(([a], [b]) => a.localeCompare(b)))); assert.equal(snapshot.indicators.L3, undefined); assert.equal(snapshot.indicators.L4, undefined); assert.equal(snapshot.scoreStatus, "not_computed_incomplete_inputs"); assert.equal(snapshot.aggregateScore, null); assert.equal(snapshot.jointState, null); assert.equal(snapshot.coverage.overall.present, 12); assert.equal(snapshot.coverage.L.present, 3); assert.equal(snapshot.coverage.overall.status, "incomplete"); }
  for (const source of Object.values(joint.sourceFiles)) { assert.match(source.path, /history\/(?:b[1-5]|f[1-4]|l[125])\.json$/); assert.match(source.sha256, /^[a-f0-9]{64}$/); }
});
